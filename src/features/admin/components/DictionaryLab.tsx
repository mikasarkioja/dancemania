...
export interface DictionaryLabProps {
  videos: { id: string; title: string; video_url: string; motion_dna: unknown }[];
}
...
export function DictionaryLab({ videos }: DictionaryLabProps) {
  ...
  const chartData = useMemo(() => {
    if (!signature) return [];
    const countLength = 8;
    const n = Math.max(signature.hipTiltCurve.length, signature.footVelocityCurve.length);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">3D playback (gold standard)</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Hip tilt &amp; foot velocity (8-count)</CardTitle>
          </CardHeader>
          <CardContent>
            {chartData.length > 0 ? (
