...
export default async function PracticeVideoPage({
  ...
  const { data: row, error } = await supabase
    .from("dance_library")
    .select("id, title, video_url, instructions, motion_dna, genre, bpm")
    .eq("id", videoId)
    .eq("status", "published")
    .single();
  ...
  const rawInstructions = Array.isArray(row.instructions)
    ? (row.instructions as MoveSegment[])
    : [];
  ...
  const hasMotionData =
    motionDna?.frames &&
    Array.isArray(motionDna.frames) &&
    motionDna.frames.length > 0;
      ...
      {hasMotionData ? (
        ...
        <>
          <p className="text-sm text-muted-foreground mb-4">
            This video doesn’t have pose data yet. Watch and follow along;
            webcam comparison will be available once the video is processed.
          </p>
          <PracticePlayer
            videoId={row.id}
            title={row.title}
